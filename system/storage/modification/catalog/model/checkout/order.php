...
class ModelCheckoutOrder extends Model {
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false, $gls_tracking = false) {
		...
		if ($order_info) {
            ...
            if(strlen($gls_tracking) > 1 && is_numeric($gls_tracking)) {
				$this->db->query("UPDATE `" . DB_PREFIX . "order` SET gls_track = '" . $gls_tracking . "' WHERE order_id = '" . (int)$order_id . "'");
			}
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				$data['title'] = sprintf($language->get('text_new_subject'), $order_id);
