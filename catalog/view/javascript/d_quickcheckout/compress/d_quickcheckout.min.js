...
qc.Login = qc.Model.extend({
    ...
    updateAccount: function(account) {
        ...
        var that = this;
        $.post('index.php?route=extension/d_quickcheckout/login/updateAccount', {
...
qc.FieldView = qc.View.extend({
    ...
    validateTelephone: function(e) {
        //alert("OKKK => " + $('.telephone-validation').length);
        ...
        if($('.phone-validation').length !== 0) {
            ...
            if ($.trim($(e.currentTarget).val())) {
                    ...
                    if($.isNumeric($(e.currentTarget).val()) == false)
                    {   

                         $(e.currentTarget).parents('.text-input').removeClass("has-error").find('.text-danger').remove();
                         $(e.currentTarget).parents('.text-input').addClass("has-error");
                         $(e.currentTarget).after('<div id=\"' + $(e.currentTarget).attr('id') + '-error\" class=\"text-danger\">' + $(e.currentTarget).data('msg-telephone') + '</div>');
                         preloaderStop();
                    }
...
qc.PaymentAddress = qc.Model.extend({
    ...
    changeAddress: function(address_id) {
        ...
        var json = this.toJSON();
        var that = this;
        $.post('index.php?route=extension/d_quickcheckout/payment_address/update', {
...
qc.PaymentAddressView = qc.View.extend({
    ...
    setZone: function(country_id) {
        var that = this;
        $.post('index.php?route=extension/d_quickcheckout/field/getZone', {
...
qc.ShippingAddress = qc.Model.extend({
    ...
    changeAddress: function(address_id) {
        ...
        var json = this.toJSON();
        var that = this;
        $.post('index.php?route=extension/d_quickcheckout/shipping_address/update', {
...
qc.Confirm = qc.Model.extend({
    ...
    updateField: function(name, value) {
        ...
        var json = this.toJSON();
        var that = this;
        $.post('index.php?route=extension/d_quickcheckout/confirm/updateField', {
    ...
    recreateOrder: function() {
        $.post('index.php?route=extension/d_quickcheckout/confirm/recreateOrder', '', function(data) {}, 'json').error();
    }
});
